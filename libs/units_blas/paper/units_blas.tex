...
Dimensional analysis is ubiquitous in the sciences.  However, in
scientific computing, attempts are seldom made to bring dimensional
analysis into code that uses or produces values with physical
dimensions.  The Boost.Units library was created to address this
frequent shortcoming.  However, the Boost.Units library is not
interoperable with linear algrbra libraries like Eigen and
Boost.uBlas, even though linear algebra is common in the scientific
computing domain.

Moreover, scientists often dismiss the importance of dimensional
analysis in linear algebra, but this paper shows that the rules that
apply to purely numeric matrices differ nontrivially from those that
apply to matrices containing values with physical dimensions.

Consider an example.  Here is the matrix whose confusing properties
motivated this paper, the x-y-doppler correlation matrix:
...
its physical dimensions preserved (the ``hat'' here indicates that
the behatted matrix contains values with physical dimensions)
...
This paper attempts to answer why this result, and other unexpected
...
\begin{cor}If an $n \times n$ dimensional value matrix $\hat M$ has
left- and right-identities $\hat I_L$ and $\hat I_R$, then
$\hat I_R = \hat I_L$ iff $\hat I_R = \hat I_L = I$. \end{cor}
