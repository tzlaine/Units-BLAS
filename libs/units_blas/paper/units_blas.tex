...
\begin{thm} For an $n \times n$ dimensional value matrix $\hat M$, the
$\hat M * inverse(\hat M) = \hat I_L$. \end{thm}
...
\[ DM = \left[ \begin{matrix} 
  ...
  \vdots & \vdots & \ddots & \vdots \\
  v_{n1} & v_{n2} & \cdots & v_{nn}
\end{matrix} \right] \] in which each $v$ is a DCV.

Applying Definition \ref{inverse_type_definition}, the matrix that contains
the DCVs of all of the values in $inverse(\hat M)$ is $DM_{Inv}$:
...
The matrix that contains the DCVs of all of the values in $\hat M * inverse(\hat M)$
$DM_{IL}$ is thus:
...
Finally, the matrix that contains the DCVs of all of the values in
$(\hat M * inverse(\hat M)) * \hat M$ is:
...
Since this result has the same dimensions as $DM$, the expression $\hat M *
inverse(\hat M)$ is a left-identity for $\hat M$. \end{proof}
