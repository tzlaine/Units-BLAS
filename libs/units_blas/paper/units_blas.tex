...
\begin{proof}This theorem can be proven by induction on the size of
 $\hat M$.  Consider a $2 \times 2$ matrix $\widehat{M2}$, and the matrix
  $DM2$ that contains the DCVs of all of the values in $\widehat{M2}$:

\[ \widehat{M2} = \left[ \begin{matrix} 
  x & y \\
  z & w
 \end{matrix} \right] , \]

\[ DM2 = \left[ \begin{matrix} 
  dcv(\langle x \rangle) & dcv(\langle y \rangle) \\
  dcv(\langle z \rangle) & dcv(\langle w \rangle)
 \end{matrix} \right] . \]
...
\[ DM2 = \left[ \begin{matrix} 
  v_{11} & v_{11} + v_{12} \prime \\
  v_{21} & v_{21} + v_{22} \prime
 \end{matrix} \right] , \]

where for each $v_{ij} \prime = v_{ij} - v_{i1}$.  Since $\widehat{M2}$ has a left-identity, each column of $\widehat{M2}$ has
the same DDV, by Theorem \ref{if_IL_then_same_col_ddvs}.  Thus,
$v_{12} \prime = v_{22} \prime$, and

\[ DM2 = \left[ \begin{matrix} 
  v_{11} & v_{11} + v_{12} \prime \\
  v_{21} & v_{21} + v_{12} \prime
 \end{matrix} \right] . \]

Notice that not only do both columns have the same DDV as each other,
but also that both rows have the same DDV as each other.  By Theorem
\ref{if_same_row_ddvs_then_IR}, $\widehat{M2}$ then has a right-identity as well.

Assume that this theorem is true for an $n \times n$ dimensional value
matrix, $\widehat{MN}$:

\[ \widehat{MN} = \left[ \begin{matrix} 
  MN_{11} & MN_{12} & \cdots & MN_{1n} \\
  MN_{21} & MN_{22} & \cdots & MN_{2n} \\
  \vdots & \vdots & \ddots & \vdots \\
  MN_{n1} & MN_{n2} & \cdots & MN_{nn}
 \end{matrix} \right] , \mbox{and} \]

\[ DMN = \left[ \begin{matrix} 
  dcv(\langle MN_{11} \rangle) & dcv(\langle MN_{12} \rangle) & \cdots & dcv(\langle MN_{1n} \rangle) \\
  dcv(\langle MN_{21} \rangle) & dcv(\langle MN_{22} \rangle) & \cdots & dcv(\langle MN_{2n} \rangle) \\
  \vdots & \vdots & \ddots & \vdots \\
  dcv(\langle MN_{n1} \rangle) & dcv(\langle MN_{n2} \rangle) & \cdots & dcv(\langle MN_{nn} \rangle)
 \end{matrix} \right] , \mbox{or} \]
...
Again, since $\widehat{MN}$ has a left-identity, each column of
$\widehat{MN}$ has the same DDV.  Thus, $v_{1k} \prime = v_{2k} \prime
= \cdots =
v_{nk}$, for all $2 <= k <= n$, and
